...
export function Select({ options, className, placeholder, error, value, onChange }: SelectProps) {
    ...
    <div>
      <div className="relative">
        ...
        <RdxSelect.Root onValueChange={handleSelect} value={value}>
          ...
          text-gray-800 focus:border-gray-800 outline-none transition-all text-left relative pt-4`,
              ...
              <RdxSelect.Viewport className="p-2">
                {options.map(option => (
                  <RdxSelect.Item
                    key={option.value}
                    value={option.value}
                    className="p-2 text-gray-800 text-sm data-[state=checked]:font-bold
              outline-none data-[highlighted]:bg-gray-50 rounded-lg transition-colors"
                  >
